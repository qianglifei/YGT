...
public class FormCheck {
	...
	public static String[] check_Card_ID(String cardID) {
		if (cardID != null && !"".equals(cardID)) { // 不为空
			...
			return new String[] { "true", "身份证号码不能为空" };
		}
	...
	public static String getSex(String cardID) {
		...
		try {
			if (cardID.length()==15) {
                sex = Integer.parseInt(cardID.substring(14, 15));
            }else {
				if (cardID.equalsIgnoreCase("XXXXXXXXXXXXXXXXXX")){
					return "";
				}else {
					Log.i(TAG, "===getSex: " + cardID);
					sex = Integer.parseInt(cardID.substring(16,17));
				}
            }
            if (sex % 2 == 0) {
				return "女";
